...
export class AIJSONService {
  ...
  async generateJSONReport(
    ...
    jsonReport: JSONReportStructure | TutoringReportStructure;
    ...
    const jsonUserPrompt = this.buildJSONUserPrompt(
      moduleType,
      documents,
      studentName,
      gradeBand
    ...
    try {
      ...
      const jsonReport =
        moduleType === "tutoring"
          ? this.parseTutoringJSONResponse(
              openaiResponse,
              studentName,
              gradeBand
            )
      ...
      if (moduleType === "tutoring") {
        ...
        markdownReport = await this.generateMarkdownFromTemplate(
          jsonReport as TutoringReportStructure,
          reportFormatPrompts
        );
      } else {
        (jsonReport as JSONReportStructure).overallQC = this.calculateOverallQC(
          jsonReport as JSONReportStructure
        );
        markdownReport = this.convertJSONToMarkdown(
          jsonReport as JSONReportStructure,
          moduleType
        );
      }
      ...
      if (moduleType === "tutoring") {
        console.log(
          `📊 Tutoring schema enforced: 3 strengths, 4 challenges, strict format`
        );
      } else {
        console.log(
          `📊 QC Summary: Avg Confidence: ${
            (jsonReport as JSONReportStructure).overallQC.averageConfidence
          }, Uncertainties: ${
            (jsonReport as JSONReportStructure).overallQC.totalUncertainties
          }`
        );
      }
...
- Generate 2-4 strengths, 2-4 challenges, and 3-5 support strategies`;
    ...
    if (moduleType === "k12") {
      moduleContext =
        "**Context:** Focus on classroom behaviors, academic skills, and family-friendly language.";
    } else if (moduleType === "tutoring") {
      moduleContext =
        "**Context:** Emphasize one-on-one learning strategies, targeted skill gaps, and tutor-implementable actions.";
    } else {
      moduleContext =
        "**Context:** Prioritize academic accommodations, self-advocacy, and supports suited for college/university settings.";
    }
  ...
  private calculateOverallQC(
    report: JSONReportStructure
  ): JSONReportStructure["overallQC"] {
    const allSections = [
    ...
    const confidenceScores = allSections.map(
      (section) => section.qc.confidence
    );
    ...
    const totalUncertainties = allSections.filter(
      (section) => section.qc.uncertaintyFlag
    ).length;
  ...
  private convertJSONToMarkdown(
    jsonReport: JSONReportStructure,
    moduleType: string
  ): string {
  ...
  private parseTutoringJSONResponse(
    response: string,
    studentName?: string,
    gradeBand?: string
    ...
    try {
      ...
      if (
        ...
        throw new Error(
          "Invalid tutoring JSON structure - missing required sections"
        );
      }

      // Validate specific field requirements
      if (
        !parsed.student_overview.tldr_paragraph ||
        typeof parsed.student_overview.tldr_paragraph !== "string"
      ) {
        throw new Error(
          "student_overview.tldr_paragraph must be a non-empty string"
        );
      }

      if (
        !parsed.student_overview.paragraph ||
        !parsed.student_overview.paragraph.main_five ||
        !parsed.student_overview.paragraph.capstone_one ||
        !parsed.student_overview.paragraph.full
      ) {
        throw new Error(
          "student_overview.paragraph must contain main_five, capstone_one, and full fields"
        );
      }

      if (
        !parsed.key_support_strategies.paragraph ||
        !parsed.key_support_strategies.paragraph.full
      ) {
        throw new Error(
          "key_support_strategies.paragraph must contain required fields"
        );
      }
      ...
      const errorMessage =
        error instanceof Error ? error.message : "Unknown error";
      throw new Error(
        `Failed to parse tutoring JSON response: ${errorMessage}`
      );
    }
  }

  private async generateMarkdownFromTemplate(
    tutoringReport: TutoringReportStructure,
    reportFormatPrompts: any[]
  ): Promise<string> {
    // Get the tutoring markdown template from database
    const templateSection = reportFormatPrompts.find(
      (p) => p.section_key === "markdown_report_template_tutoring"
    );
    ...
    console.log(
      `✅ Using database template for tutoring: ${templateSection.content.length} characters`
    );
  ...
  private convertTutoringJSONToMarkdown(
    tutoringReport: TutoringReportStructure
  ): string {
    ...
    if (
      tutoringReport.documents_reviewed &&
      tutoringReport.documents_reviewed.documents &&
      tutoringReport.documents_reviewed.documents.length > 0
    ) {
      // Use actual document names from the AI response
      tutoringReport.documents_reviewed.documents.forEach((doc) => {
        markdown += `- ${doc.filename}\n`;
      });
      markdown += `\n`;

      // Add summary if provided
      if (tutoringReport.documents_reviewed.summary) {
        markdown += `${tutoringReport.documents_reviewed.summary}\n\n`;
      }
    } else {
      // Fallback if no documents in AI response
      markdown += `Documents reviewed for this assessment.\n\n`;
    }
    markdown += `---\n\n`;
    ...
    if (
      tutoringReport.student_overview.paragraph &&
      tutoringReport.student_overview.paragraph.full
    ) {
    ...
    if (
      tutoringReport.key_support_strategies.paragraph &&
      tutoringReport.key_support_strategies.paragraph.full
    ) {
    ...
    tutoringReport.strengths.slice(0, 3).forEach((strength) => {
      const whatYouSee = `${strength.what_you_see}${
        strength.evidence
          ? `<br/><sub>**evidence:** ${strength.evidence}</sub>`
          : ""
      }${
        strength.impact ? `<br/><sub>**impact:** ${strength.impact}</sub>` : ""
      }`;
    ...
    tutoringReport.challenges.slice(0, 4).forEach((challenge) => {
      const whatYouSee = `${challenge.what_you_see}${
        challenge.evidence
          ? `<br/><sub>**evidence:** ${challenge.evidence}</sub>`
          : ""
      }${
        challenge.impact
          ? `<br/><sub>**impact:** ${challenge.impact}</sub>`
          : ""
      }`;
