...
export function registerNoCacheRoutes(app: Express) {
  ...
  app.get(
    "/api/fresh-assessment-cases/:moduleType",
    noCacheHeaders,
    async (req, res) => {
      try {
        ...
        console.log(
          `Fresh assessment cases request for ${moduleType} at ${timestamp}`
        );
        ...
        res
          .status(500)
          .json({ error: "Failed to fetch fresh assessment cases" });
      }
    }
  );
  ...
  app.get(
    "/api/fresh-item-master/:caseId/:moduleType",
    noCacheHeaders,
    async (req, res) => {
      try {
        ...
        console.log(
          `Fresh item master request for case ${caseId}, module ${moduleType} at ${timestamp}`
        );

        const itemMasterData = await storage.getItemMasterData(
          caseId,
          moduleType
        );
        ...
        res
          .status(500)
          .json({ error: "Failed to fetch fresh item master data" });
      }
    }
  );

  // Standard prompt sections endpoint
  app.get("/api/prompts/sections", noCacheHeaders, async (req, res) => {
    try {
      const moduleType = (req.query.moduleType as string) || "post_secondary";
      const timestamp = Date.now();

      console.log(`Prompt sections request for ${moduleType} at ${timestamp}`);

      const prompts = await storage.getPromptSections(moduleType);
      res.json(prompts);
    } catch (error) {
      console.error("Prompt sections error:", error);
