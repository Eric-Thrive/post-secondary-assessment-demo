  ...
  app.post("/api/demo-analyze-assessment", async (req, res) => {
    try {
      ...
      if (moduleType === "tutoring") {
        console.log(
          "🧩 Using JSON-first pipeline for tutoring module demo with QC scoring"
        );
        ...
        if (moduleType === "post_secondary" || moduleType === "k12") {
          ...
          const result = await aiService.processAnalysis({
            ...
            moduleType,
            pathway,
            documents: documentContents.map(
          ...
          console.log(
            `🔑 Demo analysis - User ID from session: ${
          ...
          const caseData = {
            ...
            displayName:
              ...
              (moduleType === "k12"
                ? "K-12 Analysis"
                : "Post-Secondary Analysis"), // Fixed: Use displayName for UI display
          ...
          try {
            ...
            console.log(
              "✅ Demo assessment case created successfully:",
              caseId
            );
          } catch (dbError: any) {
            console.warn(
    ...
    async (req: Request, res: Response) => {
      ...
      try {
        ...
        if (!createdCaseId) {
          ...
          return res
            .status(500)
            .json({ error: "Failed to create assessment case" });
        }
        ...
        if (result.status === "completed") {
          console.log(
            "📝 Generating K-12 report from database item master data..."
          );

          // Get the K-12 report template
          const reportTemplates = await storage.getPromptSections(
            "k12",
            "report_format"
          );
          const k12Template = reportTemplates.find(
            (p) => p.section_key === "markdown_report_template_k12"
          );
          ...
          console.log(
            `🗄️ Retrieved ${itemMasterData.length} item master records from database for report generation`
          );
        ...
        if (result.item_master_data && result.item_master_data.length > 0) {
          console.log(
            `💾 Saving ${result.item_master_data.length} item master entries to database...`
          );

          try {
            await saveItemMasterDataToDatabase(
              result.item_master_data,
              caseId,
              studentGrade
            );
            ...
            console.error(
              "⚠️ Failed to save item master data to database:",
              dbError
            );
            // Continue anyway - the analysis is complete
          }
...
async function generateK12ReportFromItemMaster(
  ...
  if (template && template.length > 0) {
    ...
    const validatedItems = itemMasterData.filter(
      (item) =>
        item.resolution_method === "exact_match" ||
        item.validation_status === "validated"
    );
    const reviewItems = itemMasterData.filter(
      (item) =>
        item.validation_status === "partial_inference" ||
        item.resolution_method === "ai_resolved" ||
        item.validation_status === "full_inference"
    );
    const flaggedItems = itemMasterData.filter(
      (item) => item.validation_status === "flagged"
    );

    populatedTemplate = populatedTemplate.replace(
      /\[Validated Count\]/g,
      validatedItems.length.toString()
    );
    populatedTemplate = populatedTemplate.replace(
      /\[Validated count\]/g,
      validatedItems.length.toString()
        ...
        .map((item, index) => {
          ...
          const markInferredField = (value: string, isInferred: boolean) => {
            if (
              isInferred &&
              value &&
              value !== "Assessment data indicates this area of need"
            ) {
          ...
          return `#### ${index + 1}. ${
            item.canonical_key || item.item_label || "Unknown Item"
          }

**Evidence:** ${
            item.evidence_basis ||
            item.evidence ||
            "Assessment data indicates this area of need"
          }

**Teacher-Friendly Description:** ${markInferredField(
            item.item_label ||
              item.description ||
...
**Parent-Friendly Explanation:** ${markInferredField(
            item.parent_friendly_label ||
              item.plain_language_label ||
              "This area may require additional support",
            isInferred
          )}

**Observable Behaviors:** ${markInferredField(
            item.classroom_observation ||
              item.observation ||
              "Monitor for signs of difficulty in this area",
            isInferred
          )}

**Primary Support Strategy:** ${markInferredField(
            item.support_1 ||
...
**Secondary Support Strategy:** ${markInferredField(
            item.support_2 ||
              item.secondary_support ||
              "Consider alternative approaches if needed",
            isInferred
          )}

**Implementation Caution:** ${markInferredField(
            item.caution_note ||
...
---`;
    ...
    const hasValidatedPlaceholder = populatedTemplate.includes(
      "[For each validated finding"
    );
    const hasReviewPlaceholder = populatedTemplate.includes(
      "[Same format as validated findings, but with qc_flag"
    );
    ...
    console.log(
      `- Generated validated content: ${validatedContent.length} chars`
    );

    // Replace new template placeholders that match the current template
    populatedTemplate = populatedTemplate.replace(
      /\[FOR_EACH_VALIDATED_FINDING\]/g,
      validatedContent
    );
    populatedTemplate = populatedTemplate.replace(
      /\[VALIDATED_ITEMS_CONTENT\]/g,
      validatedContent
    );
    ...
    console.log(
      `- After validated replacement: ${beforeLength} -> ${afterValidated} chars`
    );
    ...
    populatedTemplate = populatedTemplate.replace(
      /\[FOR_EACH_REVIEW_FINDING\]/g,
      reviewContent
    );
    populatedTemplate = populatedTemplate.replace(
      /\[REVIEW_ITEMS_CONTENT\]/g,
      reviewContent
    );
    ...
    if (
      ...
      populatedTemplate.includes("[Same format as validated findings")
    ) {
      // Handle variations - find all matches and replace the second one
      const flaggedMatches = populatedTemplate.match(
        /\[Same format as validated findings[^\]]*\]/g
      );
      if (flaggedMatches && flaggedMatches.length > 1) {
        ...
        const secondOccurrenceIndex = populatedTemplate.lastIndexOf(
          flaggedMatches[flaggedMatches.length - 1]
        );
        populatedTemplate =
          populatedTemplate.substring(0, secondOccurrenceIndex) +
          flaggedContent +
          populatedTemplate.substring(
            secondOccurrenceIndex +
              flaggedMatches[flaggedMatches.length - 1].length
          );
      }
    }
    ...
    console.log(
      `- Final template length: ${finalLength} chars (${
        finalLength - beforeLength
      } chars added)`
    );
    ...
    console.log(
      `- Original template contains [FOR_EACH_VALIDATED_FINDING]: ${template.includes(
        "[FOR_EACH_VALIDATED_FINDING]"
      )}`
    );
    console.log(
      `- Original template contains [FOR_EACH_REVIEW_FINDING]: ${template.includes(
        "[FOR_EACH_REVIEW_FINDING]"
      )}`
    );
    console.log(
      `- Original template contains [VALIDATED_ITEMS_CONTENT]: ${template.includes(
        "[VALIDATED_ITEMS_CONTENT]"
      )}`
    );
    console.log(
      `- Original template contains [REVIEW_ITEMS_CONTENT]: ${template.includes(
        "[REVIEW_ITEMS_CONTENT]"
      )}`
    );
    console.log(
      `- Original template contains [FLAGGED_ITEMS_CONTENT]: ${template.includes(
    ...
    console.log(
      `- Length change: ${populatedTemplate.length - template.length} chars`
    );
    console.log(
      `- Still contains [FOR_EACH_VALIDATED_FINDING]: ${populatedTemplate.includes(
        "[FOR_EACH_VALIDATED_FINDING]"
      )}`
    );
    console.log(
      `- Still contains [VALIDATED_ITEMS_CONTENT]: ${populatedTemplate.includes(
        "[VALIDATED_ITEMS_CONTENT]"
      )}`
    );
    console.log(
      `- Final template preview: ${populatedTemplate.substring(0, 300)}...`
    );

    // Replace other dynamic content
    const mappingMethods = Array.from(
      new Set(
        itemMasterData.map(
          (item) => item.resolution_method || "database_lookup"
        )
      )
    );
    populatedTemplate = populatedTemplate.replace(
    ...
    const inferredItemCount = itemMasterData.filter(
      (item) =>
        item.validation_status === "partial_inference" ||
        item.validation_status === "full_inference"
    ).length;
    ...
    populatedTemplate = populatedTemplate.replace(
      ...
      itemMasterData
        .map((item) => item.academic_domain || "General academic support")
        .join(", ")
    );
    ...
    populatedTemplate = populatedTemplate.replace(
      ...
      itemMasterData
        .filter(
          (item) => item.domain === "social" || item.domain === "behavioral"
        )
        .map((item) => item.description)
        .join(", ") || "No significant concerns noted"
    );
...
${itemMasterData
  .map(
    (item, index) => `### ${index + 1}. ${item.canonical_key || item.item_label}
...
**Description:** ${
      item.item_label || item.description || "Support needed in this area"
    }
...
async function saveReportDataDirectly(
  caseId: string,
  reportData: any
): Promise<boolean> {
  try {
...
async function saveItemMasterDataToDatabase(
  itemMasterData: any[],
  assessmentCaseId: string,
  gradeBand: string
): Promise<void> {
  console.log(
    "📊 Saving K-12 item master data with cascade inference fields..."
  );
  ...
  for (const item of itemMasterData) {
    try {
      ...
      console.log(
        `✅ Saved K-12 item master with ${itemData.inference_level} inference: ${itemData.canonical_key}`
      );
    } catch (error) {
      console.error(
        `❌ Failed to save item master entry ${item.canonical_key}:`,
        error
      );
    }
  }
...
async function createAssessmentCaseDirectly(
  caseData: any
): Promise<string | null> {
  try {
    const { pool } = await import("../db");
    ...
    console.log(
      "✅ Assessment case created via direct SQL:",
      result.rows[0].id
    );
