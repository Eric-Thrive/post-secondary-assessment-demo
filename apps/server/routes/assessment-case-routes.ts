...
import { db } from "../db";
import { assessmentCases } from "@shared/schema";
import { eq, and, desc } from "drizzle-orm";
import { DEMO_CUSTOMER_ID } from "@shared/constants/environments";
import { requireAuth, requireCustomerAccess, checkReportLimit, incrementReportCount } from "../auth";
...
export function registerAssessmentCaseRoutes(app: Express): void {
    ...
    app.get("/api/demo-assessment-cases/:moduleType", requireAuth, async (req, res) => {
      try {
        ...
        const cases = await storage.getAssessmentCases(moduleType, DEMO_CUSTOMER_ID, userId);
