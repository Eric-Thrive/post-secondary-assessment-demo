...
function isDemoOperationAllowed(method: string, path: string): boolean {
  ...
  const isAllowed = allowedOperations.some((op) => {
    ...
    const pathMatches =
      typeof op.path === "string" ? op.path === path : op.path.test(path);
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.use("/api", (req, res, next) => {
    ...
    const isDemoOpAllowed =
      isDemo && isDemoOperationAllowed(req.method, pathWithoutApi);
    ...
    if (req.isDemoOperation) {
      ...
      if (req.body && !validateDemoAccess(req.body)) {
        console.error(
          `🚨 DEMO ISOLATION VIOLATION: Non-demo customer data in request`,
          {
        ...
        return res.status(403).json({
          error:
            "SECURITY VIOLATION: Demo operations are restricted to demo customer data only",
      ...
      if (
        req.body &&
        (req.path.includes("/assessment-cases") ||
          req.path.includes("/demo-assessment-cases"))
      ) {
    ...
    if (isWriteOperation && isReadOnlyEnvironment()) {
      ...
      if (isDemoOpAllowed) {
        ...
        if (isControlledAccess) {
          console.log(
            `🔒 CONTROLLED ACCESS: Demo operation approved under enhanced security mode`,
            {
              operation: `${req.method} ${req.path}`,
          ...
          if (connInfo.environment !== "demo" && !connInfo.isDemo) {
            console.error(
              `🚨 CRITICAL SECURITY VIOLATION: Demo operation attempted on non-demo environment`,
              {
                environment: connInfo.environment,
                isDemo: connInfo.isDemo,
                operation: `${req.method} ${req.path}`,
            ...
            return res.status(503).json({
              error:
                "SECURITY VIOLATION: Demo operations cannot be performed on production environments",
              environment: connInfo.environment,
          ...
          const isProdHost = productionHostPatterns.some((pattern) =>
      ...
      console.error(
        `🚨 SECURITY VIOLATION: ${req.method} ${req.path} blocked in read-only environment`,
        {
          environment: connInfo.environment,
          isDemo: connInfo.isDemo,
          operation: `${req.method} ${req.path}`,
          ...
          ip: req.ip,
          reason: isDemo
            ? "Operation not in demo allowlist"
  ...
  if (process.env.NODE_ENV === "development") {
    const { registerDebugRoutes } = await import("./debug-routes.js");
