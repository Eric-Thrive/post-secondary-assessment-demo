...
const FigmaEnhancedReportViewer: React.FC<FigmaEnhancedReportViewerProps> = ({
  ...
  const handleEditClick = (sectionId: string) => {
    ...
    console.log(
      "🔄 Navigating to Review & Edit section for:",
      sectionId,
      "Case ID:",
      currentCase?.id
    );
  ...
  const FigmaStyledReport = () => {
    ...
    const [expandedAccommodation, setExpandedAccommodation] = useState<
      string | null
    >(null);
    const [expandedAccommodationItems, setExpandedAccommodationItems] =
      useState<Set<number>>(new Set());
    const [showAccommodationsDropdown, setShowAccommodationsDropdown] =
      useState(false);
    const [currentAccommodationSlide, setCurrentAccommodationSlide] =
      useState(0);
    ...
    const handlePrint = useReactToPrint({
      contentRef: printRef,
      documentTitle: `Accommodation_Report_${
        currentCase?.display_name || "Report"
      }`,
    });
    ...
    const getSectionForView = (view: string) => {
      ...
      console.log(
        "📋 Available sections:",
        sections.map((s) => ({ title: s.title, index: s.index }))
      );
      ...
      switch (view) {
        case "document-review":
          ...
          section = sections.find(
            (s) =>
              ...
              s.title.toLowerCase().includes("student") ||
              s.index === 1
          );
          break;
        case "functional-impact":
          ...
          section =
            sections.find(
              (s) =>
                (s.title.toLowerCase().includes("functional") &&
      ...
      console.log(
        ...
        ":",
        section
          ? { title: section.title, contentLength: section.content.length }
          : "null"
      );
      return section;
    };
      ...
      >
        ...
        <div className="relative z-10">
          ...
          <div className="fixed top-0 left-80 right-0 z-50">
            <header
              className="text-white py-3 shadow-2xl"
              style={{
                background: `linear-gradient(to right, ${brandColors.primary}, ${brandColors.lightBlue})`,
              }}
            >
          ...
          <div className="flex pt-0">
            ...
            <aside
              className="fixed left-0 top-0 h-screen w-80 bg-white shadow-xl z-40 overflow-y-auto"
                    ...
                    >
                      <div className="space-y-5">
                        ...
                        <div className="flex items-baseline">
                          ...
                          >
                            {(currentCase as any)?.program_major ||
                              (currentCase?.module_type === "post_secondary"
                                ? "Post-Secondary Program"
                                : currentCase?.module_type === "k12"
                                ? `Grade ${
                                    (currentCase as any)?.grade_band || "K-12"
                                  }`
                                : currentCase?.module_type) ||
                              "Not specified"}
                          </p>
                        </div>
                        <div className="flex items-baseline">
                          ...
                          >
                            {(currentCase as any)?.report_author ||
                      ...
                      <div className="p-4 space-y-3">
                        {(() => {
                          ...
                          const documents =
                            ...
                            (currentCase as any)?.documentNames?.map(
                              (filename: string, index: number) => ({
                                id: `doc_${index}_${filename.replace(
                                  /[^a-zA-Z0-9]/g,
                                  "_"
                                )}`,
                                name: filename,
                                ...
                                uploadDate:
                                  (currentCase as any)?.created_date ||
                                  new Date().toISOString(),
                              })
                            ) ||
                            (currentCase as any)?.document_names?.map(
                              (filename: string, index: number) => ({
                                id: `doc_${index}_${filename.replace(
                                  /[^a-zA-Z0-9]/g,
                                  "_"
                          ...
                          return documents && documents.length > 0 ? (
                            documents.map((doc: DocumentFile) => {
                              const isLastItem =
                                documents.indexOf(doc) === documents.length - 1;
                              return (
                                <div
                                  key={doc.id}
                            ...
                            <div className="text-center py-8 text-gray-500">
                              ...
                              <p className="text-sm">
                                Documents will appear here once uploaded and
                                processed
                              </p>
                            </div>
                          );
                  ...
                  <div className="h-[600px] relative">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <button
                          ...
                          onClick={(e) => {
                            ...
                            e.stopPropagation();
                            console.log(
                              "🖊️ Edit button clicked for functional-impact - navigating to Review & Edit"
                            );
                    ...
                    >
                      ...
                      >
                        {(() => {
                          const section =
                            getSectionForView("functional-impact");

                          if (!section)
                            return (
                              <div className="text-gray-500">
                                No functional impact section found
                              </div>
                            );
                          ...
                          const parsedBarriers =
                            parseFunctionalImpactBarriers(contentToDisplay);
                          ...
                          const barriers = parsedBarriers.map((b) => {
                            ...
                            return {
                              ...b,
                              title: stripMarkdownFormatting(
                                b.description ||
                                  b.title ||
                                  `Barrier ${b.number}`
                              ), // Use description as title, strip markdown
                              icon,
                            };
                          });
                          ...
                          const totalSlides = Math.ceil(
                            barriers.length / barriersPerSlide
                          );
                        ...
                        if (barrierMatch) {
                          ...
                          if (evidenceMatch) {
                            ...
                            evidence = evidenceMatch[2]
                              .replace(
                                /\s*\(flagged for view demo mode\).*$/i,
                                ""
                              )
                              .replace(
                                /\s*\*\(Flagged for review-demo mode\).*$/i,
                                ""
                              )
                              .replace(
                                /\s*\*\(Flagged for Review - Demo Mode\).*$/i,
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <button
                          onClick={(e) => {
                            ...
                            e.stopPropagation();
                            console.log(
                              "🖊️ Edit button clicked for accommodations - navigating to Review & Edit"
                            );
                      ...
                      >
                        {(() => {
                          ...
                          if (!section)
                            return (
                              <div className="text-gray-500">
                                No accommodations section found
                              </div>
                            );

                          // Debug: Log the section content
                          console.log(
                            "Accommodations section content:",
                            section.content
                          );
                          ...
                          const totalAccommodationSlides = Math.ceil(
                            accommodations.length / accommodationsPerSlide
                          );
                          const currentAccommodations = accommodations.slice(
                            currentAccommodationSlide * accommodationsPerSlide,
                            (currentAccommodationSlide + 1) *
                              accommodationsPerSlide
                          );
                            ...
                            >
                              ...
                              >
                                {currentAccommodations.map(
                                  (accommodation, slideIndex) => {
                                    const globalIndex =
                                      currentAccommodationSlide *
                                        accommodationsPerSlide +
                                      slideIndex;
                                    const isExpanded =
                                      expandedAccommodationIndex ===
                                      globalIndex;
                                    const Icon =
                                      slideIndex === 0
                                        ? Clock
                                    ...
                                    return (
                                      ...
                                      >
                                          ...
                                          <div className="flex items-center justify-between">
                                            <div className="flex items-center gap-3">
                                              <div
                                                ...
                                                style={{
                                                  backgroundColor: `${brandColors.skyBlue}30`,
                                                }}
                                              >
                                                <Icon
                                                  className="h-5 w-5"
                                        ...
                                        {isExpanded && (
                                          <div
                                            className="border-t px-4 pb-4 bg-white shadow-lg rounded-b-lg"
                                            style={{ borderColor: "#E0E0E0" }}
                                          >
                      ...
                      return (
                        <div className="absolute bottom-6 left-6 right-6 flex justify-between items-center z-20">
                          ...
                          {totalAccommodationSlides > 1 && (
                            <div className="text-sm text-gray-600">
                              Page {currentAccommodationSlide + 1} of{" "}
                              {totalAccommodationSlides}
                            </div>
                          )}
                          ...
                          <div className="flex gap-2">
                            ...
                            {(!totalAccommodationSlides ||
                              ...
                              <button
                                ...
                                onClick={() =>
                                  setCurrentView("report-complete")
                                }
                              >
                                Finished
                                <ChevronRight className="h-4 w-4" />
    ...
    return (
      <div className="p-8 text-center">
        <p className="text-gray-600">
          No analysis result available for this case.
        </p>
      </div>
    );
  }
