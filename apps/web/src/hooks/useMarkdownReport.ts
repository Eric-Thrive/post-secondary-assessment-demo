...
export const useMarkdownReport = (currentCase: AssessmentCase | null) => {
  ...
  const getMarkdownReport = (case_: AssessmentCase | null): string | null => {
    ...
    // Primary: Check for markdown_report in analysis result (object format)
    ...
    // Secondary: Check if analysis_result is a string (tutoring format)
    if (typeof analysisResult === 'string' && analysisResult.trim()) {
      const reportLength = analysisResult.length;
      console.log('Found string analysis_result (tutoring format), length:', reportLength.toLocaleString());
      console.log('Report preview (first 100 chars):', analysisResult.substring(0, 100));
