...
export class AIApiClient {
  ...
  static async testConnection(): Promise<boolean> {
    try {
      ...
      if (!response.ok) {
        console.error('❌ Server API test failed:', response.status, response.statusText);
        const errorText = await response.text();
        console.error('Error response:', errorText);
        throw new Error(`Server API not ready: ${response.status} - ${errorText}`);
      }
  ...
  static async callAnalysisAPI(request: AIAnalysisRequest): Promise<any> {
    ...
    try {
      ...
      console.log('Making POST request to server API:', `${this.SERVER_API_URL}/analyze-assessment`);
      
      response = await fetch(`${this.SERVER_API_URL}/analyze-assessment`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-Environment': currentEnvironment, // Pass environment in header
        },
        body: JSON.stringify({
          ...request,
      ...
      try {
        ...
        throw new Error(`Analysis API returned invalid JSON: ${jsonError instanceof Error ? jsonError.message : 'JSON parsing failed'}`);
      }
      ...
      if (!result || typeof result !== 'object') {
        console.warn('⚠️ Unexpected analysis response structure, using fallback');
        return {
          status: 'completed',
          analysis_date: new Date().toISOString(),
          markdown_report: '# Analysis Completed\n\nAnalysis completed but response structure was unexpected.',
