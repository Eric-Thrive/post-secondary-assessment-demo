 ...
 */
export function parsePostSecondaryReportSections(
  markdownReport: string
): ReportSection[] {
  if (!markdownReport || typeof markdownReport !== "string") {
    console.log(
      "❌ No markdown report provided or invalid type:",
      typeof markdownReport
    );
    return [];
  }
  ...
  console.log(
    "📄 Contains functional:",
    markdownReport.toLowerCase().includes("functional")
  );
  console.log(
    "📄 Contains impact:",
    markdownReport.toLowerCase().includes("impact")
  );
  console.log(
    "📄 Contains accommodation:",
    markdownReport.toLowerCase().includes("accommodation")
  );
  ...
  if (parts.length > 1) {
    parts.forEach((part, index) => {
      ...
      console.log(
        `📋 Strategy A found section ${index + 1}: "${title}" (${
          content.length
        } chars)`
      );
    });
  ...
  if (sections.length === 0 && processedReport.match(/SECTION \d+:/i)) {
    ...
    parts.forEach((part, index) => {
      ...
      console.log(
        `📋 Strategy B found section ${index + 1}: "${title}" (${
          content.length
        } chars)`
      );
    });
  ...
  if (sections.length === 0 && processedReport.includes("### ")) {
    ...
    parts.forEach((part, index) => {
      ...
      console.log(
        `📋 Strategy C found section ${index + 1}: "${title}" (${
          content.length
        } chars)`
      );
    });
  ...
  sections.forEach((section, i) => {
    ...
    if (
      section.title.toLowerCase().includes("functional") ||
      section.title.toLowerCase().includes("impact")
    ) {
      console.log(
        `📋 Functional Impact section content preview:`,
        section.content.substring(0, 200)
      );
    }
    if (section.title.toLowerCase().includes("accommodation")) {
      console.log(
        `📋 Accommodations section content preview:`,
        section.content.substring(0, 200)
      );
    }
  });
 ...
 */
export function parseAccommodationSubsections(
  ...
  if (parts.length > 1) {
    ...
    for (let i = 1; i < parts.length; i++) {
      ...
      const content = subsectionContent
        .substring((titleMatch ? titleMatch[1] : rawTitle).length)
        .trim();
  ...
  const requiredCategories = [
    {
      ...
      defaultContent:
        "Academic accommodations will be determined based on individual needs.",
    },
    {
      id: "3.2",
      title: "Instructional / Program Accommodations",
      defaultContent:
        "Instructional and program accommodations will be determined based on individual needs.",
    },
    {
      id: "3.3",
      title: "Auxiliary Aids & Services",
      defaultContent:
        "Auxiliary aids and services will be determined based on individual needs.",
    },
    {
      id: "3.4",
      title: "Non-Accommodation Supports / Referrals",
      defaultContent:
        "Non-accommodation supports and referrals will be determined based on individual needs.",
    },
  ];
  ...
  if (subsections.length > 0 && subsections.length < 4) {
    requiredCategories.forEach((required) => {
      const exists = subsections.find((s) =>
        s.title
          .toLowerCase()
          .includes(required.title.toLowerCase().split(" ")[0])
      );
      if (!exists) {
        subsections.push({
          id: required.id,
    ...
    const contentLines = accommodationContent
      .split("\n")
    ...
    requiredCategories.forEach((category, idx) => {
      subsections.push({
        ...
        content:
          contentLines.slice(idx * quarter, (idx + 1) * quarter).join("\n") ||
  ...
  console.log(
    "Parsed subsections:",
    subsections.map((s) => ({ id: s.id, title: s.title }))
  );
  return subsections;
}
 ...
 */
export function parseFunctionalImpactBarriers(
  functionalImpactContent: string
): ParsedBarrier[] {
  if (!functionalImpactContent) return [];

  console.log(
    "🔍 Parsing functional impact content:",
    functionalImpactContent.slice(0, 500)
  );
  ...
  const numberedBoldMatch = functionalImpactContent.match(
    /\*\*\d+\.\d+\.\s*[^*]+\*\*/g
  );
  ...
  if (numberedBoldMatch && numberedBoldMatch.length > 0) {
    ...
    barrierSections.forEach((section, index) => {
      ...
      const contentAfterTitle = trimmedSection
  ...
  if (
    barriers.length === 0 &&
    functionalImpactContent.match(/^\d+\.\s*\*\*/m)
  ) {
    ...
    numberedItems.forEach((item, index) => {
      ...
      const contentAfterTitle = trimmedItem
        .substring(titleMatch[0].length)
        .trim();
      const descriptionMatch = contentAfterTitle.match(
        /^-\s*(.+?)(?=\s*\([^)]+\)\s*\[[^\]]+\]|$)/
      );
    ...
    while ((match = barrierRegex.exec(functionalImpactContent)) !== null) {
      ...
      const nextBarrierMatch = functionalImpactContent
        .slice(startIndex)
        .match(/\*\*Observed Barrier \d+:/);
      const endIndex =
        nextBarrierMatch && nextBarrierMatch.index !== undefined
          ? startIndex + nextBarrierMatch.index
          : functionalImpactContent.length;
      const barrierContent = functionalImpactContent.slice(
        startIndex,
        endIndex
      );

      // Extract functional impact
      const impactMatch = barrierContent.match(
        /\*\*Functional Impact:\*\*\s*([^\n]+(?:\n(?!- \*\*)[^\n]+)*)/
      );
      ...
      // Extract evidence
      const evidenceMatch = barrierContent.match(
        /\*\*Evidence:\*\*\s*([^\n]+(?:\n(?!- \*\*)[^\n]+)*)/
      );
      let evidence = "";
      if (evidenceMatch) {
...
export function parseAccommodations(content: string): ParsedAccommodation[] {
  ...
  for (const line of lines) {
    ...
    const accommodationMatch =
      titleColonMatch || boldTitleMatch || boldNumberMatch || simpleMatch;
    ...
    if (accommodationMatch) {
      ...
      if (titleColonMatch) {
        ...
        currentDescription = accommodationMatch[3]
          ? accommodationMatch[3].trim()
          : accommodationMatch[2];
      } else {
        // Other formats
        cleanTitle = stripMarkdownFormatting(accommodationMatch[2]);
