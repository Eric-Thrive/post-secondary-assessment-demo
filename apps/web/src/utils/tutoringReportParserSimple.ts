/**
 * Enhanced Tutoring Report Parser for Current Format
...
export interface TutoringReportData {
...
/**
 * Parse Tutoring report from current markdown format
 */
export function parseTutoringReportSimple(
  markdown: string
): TutoringReportData {
  console.log("📄 Parsing Tutoring report (simplified overview)");
  ...
  console.log("🔍 Looking for strategies section...");
  let strategiesSection = extractSection(markdown, "Key Support Strategies");
  console.log(
    "  - 'Key Support Strategies' found:",
    !!strategiesSection,
    strategiesSection?.substring(0, 100)
  );
  if (!strategiesSection) {
    strategiesSection = extractSection(markdown, "Support Strategies");
    console.log("  - 'Support Strategies' found:", !!strategiesSection);
  }
  if (!strategiesSection) {
    ...
    console.log(
      "  - 'Key Support Strategies & Accommodations' found:",
      !!strategiesSection
    );
  }
...
function parseStrategiesTable(content: string): Strategy[] {
  ...
  // If no strategies found with bold labels, try parsing as plain paragraphs
  // This handles the tutoring format where strategies are just paragraphs
  if (strategies.length === 0) {
    const paragraphs = content.split("\n\n").filter((p) => p.trim());
    paragraphs.forEach((paragraph, index) => {
      const trimmed = paragraph.trim();
      if (trimmed) {
        // Use first few words as the strategy name
        const firstSentence = trimmed.split(".")[0];
        const strategyName =
          firstSentence.length > 60 ? `Strategy ${index + 1}` : firstSentence;
