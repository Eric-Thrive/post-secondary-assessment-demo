...
import { K12CardReport } from '@/components/report/K12CardReport';
import ReactMarkdown from 'react-markdown';
import remarkGfm from 'remark-gfm';
import { Button } from '@/components/ui/button';
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogTrigger } from '@/components/ui/dialog';
import { ChevronLeft, ChevronRight, FileText, ListChecks, Info } from 'lucide-react';
...
export const ReportContent: React.FC<ReportContentProps> = ({
  ...
  // If K-12 demo mode, use the card-based report display
  if (isK12Demo && isK12Module && hasAnalysisResult && markdownReport) {
    return <K12CardReport markdownReport={markdownReport} />;
  }
              ...
              {currentCase.analysis_result && (
                <>
                  ...
                  {(currentCase.analysis_result as any)?.markdown_report && (
                    <div><strong>Markdown Length:</strong> {(currentCase.analysis_result as any).markdown_report.length.toLocaleString()} characters</div>
                  )}
                </>
              )}
            </div>
