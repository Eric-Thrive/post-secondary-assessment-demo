...
export const K12ReviewEditReports: React.FC = () => {
  ...
  const handleVersionSwitch = async (version: string) => {
    ...
    try {
      ...
      const result = await response.json();
      console.log(`🔄 Frontend received content for version ${version}:`, result.content?.substring(0, 100));
  ...
  if (isLoadingCases) {
    return (
      <div className="flex items-center justify-center py-8">
        ...
        <span className="ml-2 text-gray-500">Loading K-12 assessment cases...</span>
      </div>
    );
  }
  ...
  if (completedCases.length === 0) {
    return (
      <Alert>
        ...
        <AlertDescription>
          No completed K-12 assessment reports found. Complete an assessment first to review and edit reports.
        </AlertDescription>
      </Alert>
    );
  }
            ...
            <SelectContent>
              {completedCases.map((assessmentCase) => (
                <SelectItem key={assessmentCase.id} value={assessmentCase.id}>
                  ...
                  {' - '}{assessmentCase.report_data?.analysis_date ? new Date(assessmentCase.report_data.analysis_date).toLocaleDateString() : 'No date'}
            ...
            <Card>
              <CardHeader>
                <div className="flex items-center justify-between">
                  ...
                  {versionInfo.currentVersion !== '1.0' && versionInfo.currentVersion !== 'original' && (
                    <div className="flex items-center gap-2">
                      <FileCheck className="h-4 w-4 text-blue-500" />
                      <Badge variant="secondary">
                        {versionInfo.currentVersion === '1.0' ? 'Original' : versionInfo.currentVersion}
                      </Badge>
                    </div>
                  )}
                </div>
              ...
              <CardContent className="p-0">
                <div className="p-6">
                  <K12CardReportEditable
                    ...
                    studentName={(selectedCase as any).display_name || selectedCase.student_name || 'Student'}
                    isEditMode={true}
