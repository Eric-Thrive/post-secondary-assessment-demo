...
import { Printer, CheckCircle, XCircle, User, Lightbulb, Users, Brain, BookOpen, Target, AlertCircle } from 'lucide-react';
...
export const K12CompactReport: React.FC<K12CompactReportProps> = ({ 
  ...
  const parseStrengths = (content: string) => {
    ...
    strengthItems.forEach((item, index) => {
      ...
      if (lowerItem.includes('language') || lowerItem.includes('verbal') || 
          lowerItem.includes('speak') || lowerItem.includes('express') || 
          lowerItem.includes('communicat')) {
        categories.language.items.push(item);
      } else if (lowerItem.includes('social') || lowerItem.includes('friend') || 
                 lowerItem.includes('peer') || lowerItem.includes('collaborat') || 
                 lowerItem.includes('kind') || lowerItem.includes('empath')) {
        categories.social.items.push(item);
      } else if (lowerItem.includes('reasoning') || lowerItem.includes('problem') || 
                 lowerItem.includes('think') || lowerItem.includes('visual') || 
                 lowerItem.includes('spatial') || lowerItem.includes('creative') ||
                 lowerItem.includes('build') || lowerItem.includes('solv')) {
  ...
  const parseWhatToDoCell = (whatToDoText: string): string[] => {
    ...
    const hasSymbols = whatToDoText.includes('✓') || whatToDoText.includes('✔') || 
                       whatToDoText.includes('✘') || whatToDoText.includes('✗') || 
                       whatToDoText.includes('❌') || whatToDoText.includes('×');
    ...
    if (hasSymbols) {
      ...
      allItems.forEach(item => {
        ...
        // Check if it's a negative action (✘, ✗, ❌, or ×)
                 ...
                 trimmedItem.startsWith('❌') || trimmedItem.startsWith('×')) {
          const clean = trimmedItem.replace(/[✘✗❌×]\s*/, '').trim();
  ...
  const parseChallenges = (content: string) => {
    const challenges: Array<{ challenge: string; whatYouSee: string; whatToDo: string[] }> = [];
    
    // Check if content is formatted as a markdown table
    if (content.includes('| Challenge') && content.includes('| What You See') && content.includes('| What to Do')) {
      // Parse as markdown table with multi-row support
      const lines = content.split('\n');
      let inTable = false;
      let currentChallenge: { challenge: string; whatYouSee: string; whatToDo: string[] } | null = null;
      
      for (let i = 0; i < lines.length; i++) {
        const trimmed = lines[i].trim();
        
        // Skip header and separator lines
        if (trimmed.includes('| Challenge') || trimmed.startsWith('|--') || trimmed === '') {
          inTable = true;
        ...
        if (inTable && trimmed.startsWith('|')) {
          ...
          const cells = trimmed.split('|').map(cell => cell.trim()).filter((cell, idx) => idx > 0 && idx < 4);
  ...
  const strengths = parsedReport['Strengths'] ? parseStrengths(parsedReport['Strengths']) : null;
  const challenges = parsedReport['Challenges / Areas of Need'] ? parseChallenges(parsedReport['Challenges / Areas of Need']) : [];
      ...
      <div ref={componentRef} className="bg-white p-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-4xl font-light text-blue-600 tracking-wider">THRIVE</h1>
          </div>
        ...
        <div className="grid grid-cols-2 gap-8 mb-8">
          {/* About Student */}
          <div>
            <h3 className="text-lg font-semibold text-blue-700 mb-3">About {extractedName}</h3>
          ...
          <div>
            <h3 className="text-lg font-semibold text-blue-700 mb-3">Key Support Strategies</h3>
              ...
              {parsedReport['Key Support Strategies'] ? (
                ...
                <ul className="space-y-2">
                  <li className="flex items-start gap-2">
                    <span className="font-semibold">• Use their strengths:</span>
                    <span>oral language, kindness, collaboration, and conceptual understanding.</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <span className="font-semibold">• Support key challenges:</span>
                    <span>anxiety, processing speed, transitions, and decoding/fluency.</span>
                  </li>
                </ul>
              )}
            </div>
        ...
        {strengths && (
          <div className="mb-8">
            <h3 className="text-xl font-semibold text-blue-700 mb-4">{extractedName}'s Strengths</h3>
            <div className="grid grid-cols-3 gap-4">
              {/* Spoken Language Card */}
              <div className="bg-blue-500 text-white rounded-lg p-4">
                <h4 className="font-semibold mb-3">{strengths.language.title}</h4>
              ...
              <div className="bg-orange-500 text-white rounded-lg p-4">
                <h4 className="font-semibold mb-3">{strengths.reasoning.title}</h4>
                <div className="text-sm space-y-2">
                  <div className="mb-3">
                    <p className="font-medium mb-1">What You See:</p>
                    <ul className="space-y-1">
                      {strengths.reasoning.items.slice(0, 3).map((item, idx) => (
                        <li key={idx}>• {item}</li>
                      ))}
                    </ul>
                  </div>
        ...
        {challenges.length > 0 && (
          <div className="mb-8">
            <h3 className="text-xl font-semibold text-blue-700 mb-4">Student's Challenges</h3>
        ...
        <div className="mt-8 pt-4 border-t border-gray-200 text-center">
          <p className="text-xs text-gray-500">
            ThrivED provides education and advocacy services.
            ThrivED does not provide legal or medical advice or services in any manner or form.
          </p>
        </div>
      </div>
