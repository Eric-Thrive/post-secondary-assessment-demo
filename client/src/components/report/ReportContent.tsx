...
import { PostSecondaryStyleReport } from '@/components/report/PostSecondaryStyleReport';
import FigmaEnhancedReportViewer from '@/components/FigmaEnhancedReportViewer';
import ReactMarkdown from 'react-markdown';
import remarkGfm from 'remark-gfm';
import { Button } from '@/components/ui/button';
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogTrigger } from '@/components/ui/dialog';
import { ChevronLeft, ChevronRight, FileText, ListChecks, Info, Layout, Layers } from 'lucide-react';
...
export const ReportContent: React.FC<ReportContentProps> = ({
  ...
  const [reportView, setReportView] = useState<'enhanced' | 'compact'>('enhanced');
    ...
    if (parts.length > 1) {
      ...
      for (let i = 1; i < parts.length; i++) {
        ...
        if (title.includes('2.') && title.includes('Functional Impact')) {
          ...
          const barriers = sectionBody.split(barrierPattern).filter(b => b.trim());
          
          // Check if we actually have barriers (first element might be empty)
          const actualBarriers = barriers[0]?.trim() === '' ? barriers.slice(1) : barriers;
          ...
          if (actualBarriers.length > 7) {
            ...
            for (let pageNum = 0; pageNum < pagesNeeded; pageNum++) {
              ...
              if (pagesNeeded > 1) {
                pageContent = `## 2. Functional Impact Summary (Page ${pageNum + 1} of ${pagesNeeded})`;
              }
              pageContent += '\n\n';
              ...
              sections.push({
                title: pagesNeeded > 1 
                  ? `2. Functional Impact Summary (${startIdx + 1}-${endIdx} of ${actualBarriers.length})`
        ...
        // Check if this is section 3 (Accommodation & Support Plan)
          ...
          const combinedContent = (section32Content + section33Content + section34Content).trim();
          if (combinedContent) {
            sections.push({
  ...
  React.useEffect(() => {
    ...
    console.log('Raw analysis_result from database:', currentCase.analysis_result);
    console.log('Raw report_data from database:', (currentCase as any).report_data);
    
    const analysisResult = currentCase.analysis_result || (currentCase as any).report_data;
    if (analysisResult) {
      ...
      if (analysisResult.markdown_report) {
        console.log('Raw markdown_report length:', analysisResult.markdown_report.length);
        console.log('First 500 chars of stored markdown:', analysisResult.markdown_report.substring(0, 500));
        console.log('Last 500 chars of stored markdown:', analysisResult.markdown_report.substring(Math.max(0, analysisResult.markdown_report.length - 500)));

        // Check for potential encoding or formatting issues
        const hasSpecialChars = /[^\x20-\x7E\n\r\t]/.test(analysisResult.markdown_report);
    ...
    console.log('Processed markdownReport length (from useMarkdownReport):', markdownReport?.length || 0);
  ...
  if ((isK12Module || isTutoringModule || isPostSecondaryModule) && hasAnalysisResult && markdownReport) {
    // Extract student name from the case if available
    const studentName = (currentCase as any).student_name || 
      ...
      <div>
        ...
        {reportView === 'enhanced' ? (
          ...
          isTutoringModule ? (
            ...
            <K12CardReportEnhanced markdownReport={markdownReport} studentName={studentName} />
          )
        ) : (
          // Compact view - use K12CompactReport for both K12 and tutoring (since tutoring is K12-focused)
          <K12CompactReport markdownReport={markdownReport} studentName={studentName} reportId={currentCase.id} />
        )}
      </div>
    );
  }
  ...
  return <div className="space-y-4">
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center space-x-2">
              <div className="p-1 bg-blue-100 rounded">
                {currentSection && React.createElement(currentSection.icon, { className: "h-4 w-4 text-blue-600" })}
              </div>
              <div>
          ...
          <div className="bg-gray-50 rounded-lg p-4 h-[calc(100vh-200px)] min-h-[500px] overflow-hidden">
            ...
            >
              <ReactMarkdown 
                ...
                components={{
                  h1: ({node, ...props}) => <h1 className="text-xl font-bold mb-2 mt-2 text-center" {...props} />,
                  h2: ({node, ...props}) => <h2 className="text-lg font-semibold mb-2 mt-3 border-b border-gray-200 pb-1" {...props} />,
                  h3: ({node, ...props}) => <h3 className="text-base font-medium mb-1 mt-2" {...props} />,
                  h4: ({node, ...props}) => <h4 className="text-sm font-medium mb-1 mt-2" {...props} />,
                  p: ({node, ...props}) => <p className="mb-2 leading-normal text-sm pl-0" {...props} />,
                  ul: ({node, ...props}) => <ul className="list-disc mb-2 text-sm pl-4" {...props} />,
                  ol: ({node, ...props}) => <ol className="list-decimal mb-2 text-sm pl-4" {...props} />,
      ...
      {!hasAnalysisResult && <div className="bg-white rounded-lg shadow-sm border p-6">
          <h2 className="text-xl font-semibold text-gray-900 mb-4">Debugging Information</h2>
          <div className="bg-gray-50 rounded-lg p-4">
            <div className="text-sm text-gray-700 space-y-2">
              <div><strong>Case Status:</strong> {currentCase.status}</div>
              <div><strong>Has Analysis Result:</strong> {currentCase.analysis_result ? 'Yes' : 'No'}</div>
              {currentCase.analysis_result && <>
                  <div><strong>Analysis Result Keys:</strong> {Object.keys(currentCase.analysis_result as any).join(', ')}</div>
                  <div><strong>Analysis Status:</strong> {(currentCase.analysis_result as any)?.status || 'Unknown'}</div>
                  <div><strong>Has Markdown Report:</strong> {!!(currentCase.analysis_result as any)?.markdown_report ? 'Yes' : 'No'}</div>
                  {(currentCase.analysis_result as any)?.markdown_report && <div><strong>Markdown Length:</strong> {(currentCase.analysis_result as any).markdown_report.length.toLocaleString()} characters</div>}
                </>}
            </div>
          </div>
          <NoDataPrompt title="No analysis result available" description="The assessment analysis has not completed yet or failed to generate a report." />
        </div>}
    </div>;
};