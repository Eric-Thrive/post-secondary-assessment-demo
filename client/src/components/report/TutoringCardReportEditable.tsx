...
export const TutoringCardReportEditable: React.FC<TutoringCardReportEditableProps> = ({ 
  markdownReport, 
  studentName = "Student",
  isEditMode = false,
  onChange,
  ...
  const sections = React.useMemo(() => {
    ...
    if (sections.length === 1 && sections[0].includes('##')) {
      sections = sections[0].split(/(?=^## )/m).filter(section => section.trim());
    }
    
    // Enhanced section configuration with gradients and icons for tutoring
    ...
    sections.forEach(section => {
      ...
      if (headerMatch) {
        ...
        const matchingKey = Object.keys(sectionConfig).find(key => 
          ...
          (key === 'Challenges' && (headerText.includes('Challenge') || headerText.includes('Areas of Need'))) ||
          (key === 'Learning Profile' && headerText.includes('Profile')) ||
          (key === 'Tutoring Strategies' && (headerText.includes('Strateg') || headerText.includes('Approach'))) ||
          (key === 'Key Support Strategies' && headerText.includes('Key Support')) ||
          (key === 'Support Recommendations' && (headerText.includes('Recommendation') || headerText.includes('Support'))) ||
          (key === 'Documents Reviewed' && headerText.includes('Documents Reviewed'))
        );
  ...
  const handleSaveSection = (index: number) => {
    ...
    const newMarkdown = `# Student Support Report — Tutor Orientation\n\n` +
      updatedSections.filter(section => section.title !== 'Documents Reviewed').map(section => {
    ...
    <div className="max-w-6xl mx-auto p-6 bg-white" ref={componentRef}>
        ...
        <div className="bg-gradient-to-r from-indigo-600 to-purple-600 text-white p-8 rounded-xl shadow-lg">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-indigo-100 text-lg">Generated on {formatDate()}</p>
            </div>
      ...
      <div className="space-y-6">
        {sections.map((section, index) => {
          ...
          return (
            <Card key={index} className="shadow-lg border-0 keep-together">
              <CardHeader className={`bg-gradient-to-r ${section.gradient} text-white rounded-t-lg`}>
                <CardTitle className="flex items-center justify-between">
                  <div className="flex items-center space-x-3">
                    <div className={`p-2 rounded-lg ${section.iconBg} bg-white/20`}>
              ...
              <CardContent className="p-6">
                {isEditing ? (
                  ...
                  editedContent.includes('|') && (editedContent.includes('---') || editedContent.split('|').length > 6) ? (
                    <TableEditor
                      content={editedContent}
...
const TableEditor: React.FC<TableEditorProps> = ({ content, onChange, sectionTitle }) => {
  ...
  useEffect(() => {
    ...
    const hasTable = content.includes('|') && (content.includes('---') || content.split('|').length > 6);
    ...
    if (hasTable) {
      ...
      lines.forEach(line => {
        ...
        if (line.includes('|') && !line.match(/^\s*\|?\s*[-:]+\s*(\|\s*[-:]+\s*)*\|?\s*$/)) {
  ...
  return (
    <div className="space-y-4">
      <div className="bg-blue-50 p-3 rounded-lg">
        <h4 className="font-medium text-blue-900 mb-2">Editing: {sectionTitle}</h4>
        <p className="text-sm text-blue-700">Click on any cell to edit. Use the buttons below to add or remove rows.</p>
      </div>
...
const FormattedTextEditor: React.FC<FormattedTextEditorProps> = ({ content, onChange, sectionTitle }) => {
    ...
    <div className="space-y-4">
      <div className="bg-blue-50 p-3 rounded-lg">
        ...
        <p className="text-sm text-blue-700">
          Edit the text content below. Use **text** for bold, *text* for italic, and - for list items.
        </p>
      </div>
