...
/**
 * Comprehensive Production Database Export Script
 * 
 * This script exports all data from the production database to JSON files
 * in a structured format that can be imported into development database.
 * 
 * Features:
 ...
 * 
 * Usage:
 * DATABASE_URL=postgres://... tsx scripts/export-production-data.ts
 */
...
class DatabaseExporter {
  ...
  constructor(databaseUrl: string, config: Partial<ExportConfig> = {}) {
    ...
    this.config = {
      batchSize: 1000,
      outputDir: `/tmp/db-exports/production-${new Date().toISOString().replace(/[:.]/g, '-')}`,
