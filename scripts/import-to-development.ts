...
/**
 * Comprehensive Development Database Import Script
 * 
 * This script imports data from JSON export files into a development database.
 ...
 * 
 * Usage:
 * DEV_DATABASE_URL=postgres://... tsx scripts/import-to-development.ts /tmp/db-exports/production-2024-01-01
 */
...
class DatabaseImporter {
  ...
  constructor(devDatabaseUrl: string, importDir: string, config: Partial<ImportConfig> = {}) {
    this.pool = new Pool({ 
...
async function main() {
  ...
  if (args.length === 0) {
    console.error('❌ Usage: tsx scripts/import-to-development.ts <export-directory>');
    console.error('   Example: tsx scripts/import-to-development.ts exports/production-2024-01-01');
    process.exit(1);
  }
  ...
  console.log('🔗 Using development database URL:', devDatabaseUrl.replace(/\/\/.*:.*@/, '//***:***@'));
  ...
  const importer = new DatabaseImporter(devDatabaseUrl, importDir, config);
