...
/**
 * Test script to verify that all environments can access their appropriate prompts
 * Tests that prompts are correctly filtered by module_type from the unified database
 */
...
async function testDemoPrompts() {
  ...
  // Get the database URL (unified database with RBAC)
  const demoDbUrl = process.env.DATABASE_URL;
  ...
  try {
    ...
    if (countResult.rows.length === 0) {
      console.log("⚠️  No prompts found in the database");
    } else {
      console.log("📊 Prompt Distribution:");
      for (const row of countResult.rows) {
        promptCounts[row.module_type] = parseInt(row.prompt_count);
        console.log(
          `   ${row.module_type}: ${row.prompt_count} prompts (${row.prompt_types} types, ${row.pathway_types} pathways)`
        );
      }
    }
    console.log();
    ...
    console.log("----------------------");
    const tutoringPrompts = await testModuleAccess(
      pool,
      "tutoring",
      "Tutoring Demo"
    );
    ...
    if (tutoringPrompts === 0) {
      console.log(
        "ℹ️  Tutoring module uses JSON-based prompts (no DB prompts found)"
      );
      ...
      if (configResult.rows.length > 0) {
        ...
        console.log(
          "⚠️  No tutoring AI configuration in database (likely uses hardcoded values)"
        );
      }
    }
    console.log();
    ...
    if (unusualResult.rows.length > 0) {
      console.log(
        "⚠️  Found unexpected module types:",
        unusualResult.rows.map((r) => r.module_type).join(", ")
      );
      console.log("   This might indicate unexpected data in database");
    } else {
    ...
    for (const row of versionResult.rows) {
      console.log(
        `   ${row.module_type}: ${row.version_count} versions (latest: ${row.latest_version})`
      );
    }
    console.log();
    ...
    for (const row of typeResult.rows) {
      ...
      console.log(
        `     - ${row.prompt_type} / ${row.pathway_type}: ${row.count} prompts`
      );
    }
    console.log();
    ...
    console.log(
      `✅ Total Module Types Found: ${Object.keys(promptCounts).length}`
    );

    if (promptCounts.post_secondary > 0) {
      console.log(
        `✅ Post-Secondary Demo: ${promptCounts.post_secondary} prompts available`
      );
    } else {
    ...
    if (promptCounts.tutoring > 0) {
      console.log(
        `✅ Tutoring Demo: ${promptCounts.tutoring} prompts available`
      );
    } else {
    ...
    if (promptCounts.tutoring === 0 || !promptCounts.tutoring) {
      console.log(
        "   • Consider migrating tutoring prompts to database for consistency"
      );
    ...
    if (Object.keys(promptCounts).length < 2) {
      console.log(
        "   • Add more module types to database for comprehensive testing"
      );
    }
...
async function testModuleAccess(
  ...
  if (result.rows.length === 0) {
    console.log(`   ⚠️  No ${moduleType} prompts found in database`);
    return 0;
  }

  console.log(
    `   ✅ Found ${result.rows.length} ${moduleType} prompts (showing first 5):`
  );

  for (const prompt of result.rows as PromptInfo[]) {
    console.log(
      `      - ${prompt.section_key} (${prompt.prompt_type}/${prompt.pathway_type}) v${prompt.version}`
    );
  }
