...
/**
 * Script to update K-12 Demo report template to v10.0
 * - Removes ALL <br> tags and uses proper markdown formatting
 * - Uses newlines within table cells for proper rendering
 */

async function updateK12DemoTemplateV10() {
  console.log('🚀 Updating K-12 Demo report template to v10.0...\n');
  ...
  // New template without any <br> tags
  ...
  try {
    ...
    if (updateResult.rows.length === 0) {
      ...
      await pool.query(`
        ...
        VALUES ('markdown_report_template_k12_demo', 'K-12 Demo Markdown Report Template', 'k12', $1, 'v10.0', 'report_format')
      `, [newTemplate]);
    ...
    if (prodUpdateResult.rows.length === 0) {
      await pool.query(`
        INSERT INTO prompt_sections (section_key, section_name, module_type, content, version, prompt_type)
        VALUES ('markdown_report_template_k12', 'K-12 Markdown Report Template', 'k12', $1, 'v10.0', 'report_format')
      `, [newTemplate]);
    ...
    console.log('✅ K-12 demo report template updated to v10.0 successfully!');
    console.log('🎯 Key improvements:');
    console.log('   - Completely removed all <br> tags');
    console.log('   - Using proper markdown line breaks (double newlines)');
    console.log('   - Clean table cell formatting');
...
if (import.meta.url === `file://${process.argv[1]}`) {
  updateK12DemoTemplateV10();
}

export { updateK12DemoTemplateV10 };