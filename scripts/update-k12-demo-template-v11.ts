...
/**
 * Script to update K-12 Demo report template to v11.0
 * - Uses separate rows for ✔ and ✘ items to fix column alignment issues
 * - Cleaner table structure with better markdown compatibility
 */

async function updateK12DemoTemplateV11() {
  console.log('🚀 Updating K-12 Demo report template to v11.0...\n');
  ...
  // New template with separate rows for ✔ and ✘ items
  ...
  try {
    ...
    if (updateResult.rows.length === 0) {
      ...
      await pool.query(`
        ...
        VALUES ('markdown_report_template_k12_demo', 'K-12 Demo Markdown Report Template', 'k12', $1, 'v11.0', 'report_format')
      `, [newTemplate]);
    ...
    if (prodUpdateResult.rows.length === 0) {
      await pool.query(`
        INSERT INTO prompt_sections (section_key, section_name, module_type, content, version, prompt_type)
        VALUES ('markdown_report_template_k12', 'K-12 Markdown Report Template', 'k12', $1, 'v11.0', 'report_format')
      `, [newTemplate]);
    ...
    console.log('✅ K-12 demo report template updated to v11.0 successfully!');
    console.log('🎯 Key improvements:');
    console.log('   - Separate rows for ✔ and ✘ items');
    console.log('   - Fixed column alignment issues');
...
if (import.meta.url === `file://${process.argv[1]}`) {
  updateK12DemoTemplateV11();
}

export { updateK12DemoTemplateV11 };