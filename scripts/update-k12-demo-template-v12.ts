...
/**
 * Script to update K-12 Demo report template to v12.0
 * - Removes the Additional Notes section completely
 * - Keeps only the 4 main sections: Student Overview, Key Support Strategies, Strengths, Challenges
 */

async function updateK12DemoTemplateV12() {
  console.log('🚀 Updating K-12 Demo report template to v12.0...\n');
  ...
  // New template without Additional Notes section
  ...
  try {
    ...
    if (updateResult.rows.length === 0) {
      ...
      await pool.query(`
        ...
        VALUES ('markdown_report_template_k12_demo', 'K-12 Demo Markdown Report Template', 'k12', $1, 'v12.0', 'report_format')
      `, [newTemplate]);
    ...
    if (prodUpdateResult.rows.length === 0) {
      await pool.query(`
        INSERT INTO prompt_sections (section_key, section_name, module_type, content, version, prompt_type)
        VALUES ('markdown_report_template_k12', 'K-12 Markdown Report Template', 'k12', $1, 'v12.0', 'report_format')
      `, [newTemplate]);
    ...
    console.log('✅ K-12 demo report template updated to v12.0 successfully!');
    console.log('🎯 Key improvements:');
    console.log('   - Removed Additional Notes section completely');
    console.log('   - Report now has 4 sections: Student Overview, Key Support Strategies, Strengths, Challenges');
...
if (import.meta.url === `file://${process.argv[1]}`) {
  updateK12DemoTemplateV12();
}

export { updateK12DemoTemplateV12 };