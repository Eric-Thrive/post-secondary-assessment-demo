...
/**
 * Script to update K-12 Demo report template to v8.0
 * - Fixes line breaks in tables (no <br> tags)
 * - Adds teacher-friendly language requirement
 * - Limits what you see/do items to 7 words
 */

async function updateK12DemoTemplateV8() {
  console.log('🚀 Updating K-12 Demo report template to v8.0...\n');
  ...
  // New template with proper line breaks and updated instructions
  ...
  try {
    ...
    if (updateResult.rows.length === 0) {
      ...
      await pool.query(`
        ...
        VALUES ('markdown_report_template_k12_demo', 'K-12 Demo Markdown Report Template', 'k12', $1, 'v8.0', 'report_format')
      `, [newTemplate]);
    ...
    if (prodUpdateResult.rows.length === 0) {
      await pool.query(`
        INSERT INTO prompt_sections (section_key, section_name, module_type, content, version, prompt_type)
        VALUES ('markdown_report_template_k12', 'K-12 Markdown Report Template', 'k12', $1, 'v8.0', 'report_format')
      `, [newTemplate]);
    ...
    console.log('✅ K-12 demo report template updated to v8.0 successfully!');
    console.log('🎯 New features:');
    console.log('   - Fixed line breaks in tables (no <br> tags)');
    console.log('   - Added teacher-friendly language requirement');
    console.log('   - 7-word limit for action items');
    console.log('   - Checkmarks and X\'s on separate lines in table cells');
...
if (import.meta.url === `file://${process.argv[1]}`) {
  updateK12DemoTemplateV8();
}

export { updateK12DemoTemplateV8 };