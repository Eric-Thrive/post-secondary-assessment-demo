...
class SecurityTester {
  ...
  async testSecureConnectionString() {
    ...
    try {
      ...
      // Test that demo environments get enhanced connection strings
      process.env.APP_ENVIRONMENT = 'post-secondary-demo';
      
      // Mock a demo config since we might not have actual demo database URLs
      const mockDemoConfig = {
        url: 'postgresql://demo:demo@localhost:5432/demo_db',
        name: 'Test Demo Database',
        description: 'Test demo database',
        readOnly: true,
        isDemoEnvironment: true
      };
      
      const demoConnection = getSecureConnectionString(mockDemoConfig);
      
      this.addResult(
        'Demo Connection String Enhancement',
        demoConnection.includes('default_transaction_read_only=on'),
        'Demo environments should have read-only parameters added to connection string'
      );
  ...
  async testDatabaseWriteBlocking() {
    ...
    try {
      // Check if we have demo database URLs configured
      const hasDemoConfig = process.env.POST_SECONDARY_DEMO_DATABASE_URL || process.env.K12_DEMO_DATABASE_URL;
      
      if (!hasDemoConfig) {
        this.addResult(
          'Demo Database Write Test',
          true,
          'SKIPPED: No demo database URLs configured - this is expected and secure'
        );
        return;
      }
      
      // Test with post-secondary demo if available
      if (process.env.POST_SECONDARY_DEMO_DATABASE_URL) {
        process.env.APP_ENVIRONMENT = 'post-secondary-demo';
        await this.testWriteOperationsBlocked('Post-Secondary Demo');
      }
      
      // Test with k12 demo if available
      if (process.env.K12_DEMO_DATABASE_URL) {
        process.env.APP_ENVIRONMENT = 'k12-demo';
        await this.testWriteOperationsBlocked('K12 Demo');
      }
  ...
  private async testWriteOperationsBlocked(envName: string) {
    try {
      ...
      try {
        ...
        const isReadOnlyActive = readOnlyResult.rows[0]?.default_transaction_read_only === 'on';
        ...
        try {
          await client.query('CREATE TEMP TABLE security_test_table_write (id SERIAL PRIMARY KEY, test_data TEXT)');
          this.addResult(
          ...
          this.addResult(
            `${envName} Create Table Block`,
            writeError.message.includes('read-only') || writeError.message.includes('cannot execute'),
