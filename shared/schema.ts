...
export const users = pgTable("users", {
  ...
  role: text("role").notNull().default("tutor"), // 'system_admin', 'customer_admin', 'tutor'
  isActive: boolean("is_active").notNull().default(true),
  // Report count tracking
  reportCount: integer("report_count").notNull().default(0),
  maxReports: integer("max_reports").notNull().default(5),
  // Password reset functionality
  resetToken: text("reset_token"),
  resetTokenExpiry: timestamp("reset_token_expiry", { withTimezone: true }),
  // Registration token for admin notifications
  registrationToken: text("registration_token"),
  // Email for password reset and notifications (required and unique)
  email: text("email").notNull().unique(),
  // Demo permissions - JSON object with demo environment access
...
export const assessmentFindings = pgTable("assessment_findings", {
  id: uuid("id").primaryKey().defaultRandom(),
  assessmentCaseId: uuid("assessment_case_id").references(() => assessmentCases.id),
...
export const itemMaster = pgTable("item_master", {
  ...
  sourceFindingId: uuid("source_finding_id").references(() => assessmentFindings.id), // Link back to original finding
...
export const postSecondaryAccommodations = pgTable("post_secondary_accommodations", {
  id: uuid("id").primaryKey().defaultRandom(),
  canonicalKey: text("canonical_key").notNull(),
...
export const insertAssessmentCaseSchema = createInsertSchema(assessmentCases).omit({
  id: true,
  createdDate: true,
  lastUpdated: true,
});

export const insertPromptSectionSchema = createInsertSchema(promptSections).omit({
  id: true,
  createdAt: true,
  lastUpdated: true,
});
...
export const insertPlainLanguageMappingSchema = createInsertSchema(plainLanguageMappings).omit({
  id: true,
  createdAt: true,
  lastUpdated: true,
});

export const insertInferenceTriggerSchema = createInsertSchema(inferenceTriggers).omit({
  id: true,
  createdAt: true,
  lastUpdated: true,
});

export const insertBarrierGlossarySchema = createInsertSchema(barrierGlossary).omit({
  id: true,
  createdAt: true,
  lastUpdated: true,
});

export const insertAssessmentFindingsSchema = createInsertSchema(assessmentFindings).omit({
  id: true,
  createdAt: true,
  lastUpdated: true,
});
...
export type InsertPlainLanguageMapping = z.infer<typeof insertPlainLanguageMappingSchema>;
export type PlainLanguageMapping = typeof plainLanguageMappings.$inferSelect;

export type InsertInferenceTrigger = z.infer<typeof insertInferenceTriggerSchema>;
...
export type InsertAssessmentFindings = z.infer<typeof insertAssessmentFindingsSchema>;
...
export type PostSecondaryItemMaster = typeof postSecondaryItemMaster.$inferSelect;
export type PostSecondaryAccommodations = typeof postSecondaryAccommodations.$inferSelect;
